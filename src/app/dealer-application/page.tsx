...
function DealerApplication() {
  ...
  const {
    ...
    error,
  } = useQuery(GET_DEALER_PAGE, {
    variables: { first: 1 },
  });

  const dealerPageData: DealerPageData | null = React.useMemo(() => {
  ...
  useEffect(() => {
    if (dealerPageData?.fields) {
      const initialFormData: Record<string, string> = {};
      dealerPageData.fields.forEach((field) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (dealerPageData?.recaptchaEnabled && !recaptchaValue) {
    ...
    try {
      ...
      if (dealerPageData?.emailTo) {
        recipients.push(
          ...dealerPageData.emailTo.split(",").map((e) => e.trim())
        );
      }

      const ccRecipients: string[] = [];
      if (dealerPageData?.emailCc) {
        ccRecipients.push(
          ...dealerPageData.emailCc.split(",").map((e) => e.trim())
        );
      }

      const bccRecipients: string[] = [];
      if (dealerPageData?.emailBcc) {
        bccRecipients.push(
          ...dealerPageData.emailBcc.split(",").map((e) => e.trim())
        );
      }
      ...
      // Reset reCAPTCHA
      if (dealerPageData?.recaptchaEnabled) {
        ...
        <>
          ...
          <div className="fixed top-4 right-4 z-50 animate-slide-in">
            <Toast
              ...
              subParagraph={
                dealerPageData?.successMessage
                  ? dealerPageData.successMessage.replace(/<[^>]*>/g, "")
        ...
        <div className="flex flex-col items-start w-full gap-8 py-12 md:py-16 lg:py-24">
          <div className="flex flex-col items-start gap-5 w-full">
            <Breadcrumb
              items={[
                { text: "Home", link: "/" },
                { text: "Become a Dealer", link: "/dealer-application" },
              ]}
            />
            <Heading content={dealerPageData.title} />
          </div>
          ...
          {dealerPageData.isPublished && (
            ...
            >
              ...
              {dealerPageData.fields.some(
                ...
                <div className="w-full">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    required={dealerPageData.fields.some(
                      (field) =>
              ...
              {dealerPageData.recaptchaEnabled &&
